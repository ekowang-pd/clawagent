...
### 主路径受阻时寻找替代方案 (Workaround First)

**规则**: 当标准工具/CLI/API 反复失败时，立即探索底层替代方案，不要陷入重复重试的死循环。

**BadCase 教训**: ClawHub skill 安装  
- 错误做法：反复用 `clawhub install`，遇到限流就重试，最后只能报告失败  
- 正确做法：发现 CLI 不可靠 → 直接调用下载 API → 手动放置文件 → 完成安装

**解耦思维**:  
把「安装」拆成「获取资源 + 放置到正确位置」：
- CLI 工具只是封装，底层通常是 HTTP 下载 + 文件移动
...
**主动提案**:
- ❌ "工具装不上，一直超时"
- ✅ "CLI 限流了，我发现可以直接下载文件手动安装，要我试试吗？"

**常见替代方案思路**:
- API 限流/超时 → 直接下载、镜像源、本地缓存
- 包管理器失败 → 源码编译、二进制下载、容器化
- 网络问题 → 离线包、内网源、代理切换
