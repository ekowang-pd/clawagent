...
const kimi_finance_js_1 = require("./src/data_source/kimi_finance.js");
const DEFAULT_SEARCH_URL = "https://api.kimi.com/coding/v1/search";
const DEFAULT_FETCH_URL = "https://api.kimi.com/coding/v1/fetch";
const DEFAULT_FINANCE_URL = "https://api.kimi.com/coding/v1/tools";
...
function parseConfig(value) {
    ...
    const fetch = root.fetch && typeof root.fetch === "object" && !Array.isArray(root.fetch)
        ? root.fetch
        : {};
    const finance = root.finance && typeof root.finance === "object" && !Array.isArray(root.finance)
        ? root.finance
        : {};
    const commonHeaders = {
        ...defaultMshHeaders(),
    ...
    return {
        ...
        finance: {
            baseUrl: typeof finance.baseUrl === "string" && finance.baseUrl.trim()
                ? finance.baseUrl.trim()
                : DEFAULT_FINANCE_URL,
            timeoutSeconds: typeof finance.timeoutSeconds === "number" && Number.isFinite(finance.timeoutSeconds)
                ? Math.max(1, Math.floor(finance.timeoutSeconds))
                : 30,
            customHeaders: parseStringMap(finance.customHeaders),
            enableService: typeof finance.enableService === "boolean" ? finance.enableService : true,
        },
    };
}
...
const financeParameters = {
    ...
    properties: {
        ticker: {
            type: "string",
            description: "Stock code(s), comma-separated, maximum 3. A-share only. e.g. '600223.SH' or '000001.SZ,600519.SH'",
        },
        time: {
            type: "string",
            description: "Query time in format YYYY-MM-DD HH:MM:SS (seconds must be 00). e.g. '2024-12-25 14:30:00'",
        },
        type: {
            type: "string",
            enum: ["open_summary", "close_summary", "realtime_price", "realtime_tech"],
            default: "realtime_price",
            description: "Query type: open_summary (opening data), close_summary (closing data), realtime_price (real-time fundamental data), realtime_tech (real-time technical indicators)",
        },
        file_path: {
            type: "string",
    ...
    required: ["ticker", "time", "file_path"],
};
const plugin = {
    id: "kimi-search",
    name: "Kimi Search",
    ...
    configSchema: {
        ...
        uiHints: {
            ...
            "finance.baseUrl": { label: "Finance URL" },
            "finance.timeoutSeconds": { label: "Finance Timeout (s)", advanced: true },
            "finance.enableService": { label: "Enable Finance Service" },
        },
    },
    register(api) {
        ...
        api.registerTool({
            name: "kimi_finance",
            label: "Kimi Finance",
            description: "Get real-time stock price data from ifind with four different query types: opening summary, closing summary, real-time price, and real-time technical indicators. Supports A-share stocks only (max 3 codes per query).",
            ...
            async execute(toolCallId, params) {
                ...
                const ctx = {
                    ...
                    userAgent: config.userAgent,
                    config: {
                        baseUrl: config.finance.baseUrl,
                        apiKey: config.apiKey,
                        timeoutSeconds: config.finance.timeoutSeconds,
                        customHeaders: {
                            ...config.commonHeaders,
                            ...config.finance.customHeaders,
                        },
                    },
                };
                const result = await (0, kimi_finance_js_1.executeFinance)(typed, ctx);
                return toToolResponse(result);
            },
        });
        if (!config.apiKey) {
            api.logger.warn("[kimi-search] API key is missing. Set KIMI_PLUGIN_API_KEY.");
        }
    },
};
